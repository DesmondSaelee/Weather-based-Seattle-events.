...
function musicEvent() {
  ...
  fetch(
    `https://concerts-artists-events-tracker.p.rapidapi.com/artist?name=${artistName}&page=1`,
    options
  )
    ...
    <p>Peformance Date: ${valuesCard.startDate}</p>
      ...
      artistCardEl.innerHTML = returnCards(response);
    })
    .catch(function (err) {
      openModal(err);
    });
...
function getConcertDetails() {
  fetch(
    "https://concerts-artists-events-tracker.p.rapidapi.com/venue?name=Hollywood%20bowl&page=1",
    rapidApiOptions
  )
    .then(function(response){
        if(response.ok){
...
function createPlaceDetailCard(dataObject, category) {
  ...
  for (let x = 0; x < dataObject.categories.length; x++) {
    ...
    if (dataObject.categories[x] === category) {
      ...
      cardContent.append(
        $(
          `<p>${category} type: ${
            dataObject.categories[x + 1].split(".")[1]
          }</p>`
        )
      );
    }
  ...
  if (dataObject.phone !== undefined) {
    ...
    let phoneNumberFormatted = dataObject.phone
      .replace("(", "")
      .replace(")", "")
      .replace("-", "");
    let phoneLink = $(
      `<a href="tel:${phoneNumberFormatted}">${dataObject.phone}</a>`
    );
    if (category === "commercial") {
